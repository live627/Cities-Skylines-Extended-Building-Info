        ...
        public void Update()
        {
            ...
            if (enabled && info.isVisible && BuildingManager.instance != null && ((SimulationManager.instance.m_currentFrameIndex & 15u) == 15u || lastSelected != buildingId))
            {
                ...
                if (ai is FireStationAI)
                {
                    ...
                    strs[4] = Localization.Get(LocalizationCategory.ServiceInfo, "FireFighting") + ": " + strength.ToString();
                    var radius = ((FireStationAI)ai).m_fireDepartmentRadius / 8;
                    strs[5] = Localization.Get(LocalizationCategory.ServiceInfo, "Radius") + ": " + radius.ToString("F0");
                    strs[6] = Localization.Get(LocalizationCategory.ServiceInfo, "KittensSaved") + ": " + GetLlamaSightings(1.4);
                }
                else if (ai is MonumentAI)
                {
                    var strength = ((MonumentAI)ai).m_entertainmentAccumulation * productionRate / 100;
                    strs[4] = Localization.Get(LocalizationCategory.ServiceInfo, "Entertainment") + ": " + strength.ToString();
                    ...
                    strs[6] = Localization.Get(LocalizationCategory.ServiceInfo, "Attractiveness") + ": " + tourism.ToString();
                }
                else if (ai is HospitalAI)
                {
                    var strength = ((HospitalAI)ai).m_healthCareAccumulation * productionRate / 100;
                    strs[4] = Localization.Get(LocalizationCategory.ServiceInfo, "Healthcare") + ": " + strength.ToString();
                ...
                else if (ai is CemeteryAI)
                {
                    ...
                    strs[4] = Localization.Get(LocalizationCategory.ServiceInfo, "Deathcare") + ": " + strength.ToString();
                ...
                else if (ai is ParkAI)
                {
                    ...
                    strs[5] = Localization.Get(LocalizationCategory.ServiceInfo, "Radius") + ": " + radius.ToString();
                    strs[6] = Localization.Get(LocalizationCategory.ServiceInfo, "Llamas") + ": " + GetLlamaSightings(2);
                }
                else if (ai is SchoolAI)
                {
                    var strength = ((SchoolAI)ai).m_educationAccumulation * productionRate / 100;
                    strs[4] = Localization.Get(LocalizationCategory.ServiceInfo, "Education") + ": " + strength.ToString();
                    var radius = ((SchoolAI)ai).m_educationRadius / 8;
                    strs[5] = Localization.Get(LocalizationCategory.ServiceInfo, "Radius") + ": " + radius.ToString("F0");
                    strs[6] = Localization.Get(LocalizationCategory.ServiceInfo, "ClassesSkipped") + ": " + GetLlamaSightings(2);
                }
                else if (ai is PoliceStationAI)
                {
                    var strength = ((PoliceStationAI)ai).m_policeDepartmentAccumulation * productionRate / 100;
                    strs[4] = Localization.Get(LocalizationCategory.ServiceInfo, "Police") + ": " + strength.ToString();
                ...
                else if (ai is CargoStationAI)
                {
                    ...
                    strs[4] = Localization.Get(LocalizationCategory.ServiceInfo, "Cargo") + ": " + strength.ToString();
